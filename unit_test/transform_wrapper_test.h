...
TEST (transform_wrapper_test, forward_transform)
{
    ...
    ASSERT_EQ (output [0].imag (), 0);
    ASSERT_EQ (output [1].real (), 1.5);
    ASSERT_EQ (output [1].imag (), -4.5);
    ASSERT_EQ (output [2].real (), 8);
    ASSERT_EQ (output [2].imag (), 0);
    ASSERT_EQ (output [3].real (), 1.5);
    ASSERT_EQ (output [3].imag (), 4.5);
